#' @importFrom stats lm model.frame model.response optim pnorm
...
sfa <- function(formula, data = NULL,   pars = NULL,
  ...
  ### b0
      ...
      log_loss <- function(p) {
        # p are parameters here
        b <- p[3:length(p)]
        sigmau2_dmu <- p[1]
        sigmav2 <- p[2]
        #Nd <- length(y)
        residuals <- (y - X%*%b)
        ret = (nobs * log(sqrt(2) / sqrt(pi)) + nobs * log(1 / (sqrt(sigmau2_dmu + sigmav2)))
               + sum(log(pnorm(-sc*(residuals * (sqrt(sigmau2_dmu) / sqrt(sigmav2))) /
                                 (sqrt(sigmau2_dmu + sigmav2)))))
               - 1 / (2 * (sigmau2_dmu + sigmav2)) * sum(residuals^2))
        names(ret) <- "Log-Lik normal/half-normal distribution"
        return(-ret)
      }
      q <- optim(par = pars, fn = log_loss, method = "L-BFGS-B",
                   lower = c(.01, .01, rep(-Inf, nvar)), hessian = TRUE )
#L-BFGS-B functie kan uitgebreid worden
      coef <- q$par
