...
The data set consists of `r nrow(params$data)` observations and `r ncol(params$data)` variables.
...
```{r, echo=FALSE}
tbl <- summary_tbl_dea(params$model$eff)
knitr::kable(tbl)
```

```{r, echo=FALSE}
x <- summary(params$model$eff)
...
```{r, echo=FALSE}
d <- data.frame(
  `Efficiency score` = round(params$model$eff, params$settings$digits),
  check.names = FALSE
)
knitr::kable(d)
```
...
model <- Benchmarking::dea(
  x, y, RTS = rts, ORIENTATION = orientation
)
...
slack <- Benchmarking::slack(x, y, model)
