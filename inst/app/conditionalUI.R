...
output$ui.subset.info <- renderUI({
  ...
  n_rows <- length(input$preview_rows_selected)
  t_rows <- nrow(data()$file)
  txt <- ifelse(n_rows == 1,
                paste(n_rows, 'row of', t_rows, 'is'),
                paste(n_rows, 'rows of', t_rows, 'are'))
  
  tagList(
    p(class = 'small', helpText(paste(
      'Click to subset data. You can select the rows to include in the analysis by',
      'clicking the rows in the table. Currently', txt, 'selected.'
    ))),
    actionButton('data.subset.select', 'Select all', size = 'sm', color = 'dark', outline = TRUE)
  )
