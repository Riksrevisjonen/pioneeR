...
shinyServer(function(input, output, session) {
  ...
  onRestore(function(state) {
    restoreVals$data <- state$values$data
  ...
  data <- reactive({
    ...
    # if (is.null(input$datafile)) return(list(file = NULL, cols = NULL, error = NULL))
    if (is.null(input$datafile) && is.null(restoreVals$data))
      return(list(file = NULL, cols = NULL, error = NULL))
    
    if (!is.null(restoreVals$data))
      f <- restoreVals$data
    else
      f <- fileImport(input$datafile$datapath)
  ...
  output$data.preview <- renderUI({
    ...
    if (is.null(data()$file) && !is.null(data()$error)) {
      dashboardCard(
        header = 'Error!',
        paste(
          'An error occured while reading the file. Please try another file.'
        )
      )
      ...
      dashboardCard(
        ...
        list(
          ...
          actionButton('goToHelp', 'Click here for more information')
        )
      )
    } else {
      list(
  ...
  observeEvent(input$goToHelp, {
    updateNavbarPage(session, 'pioneer', 'pioneer_help')
  })
  ...
  # output$dea.static.plot <- renderPlot({
  #   req(data(), input$dea.input, input$dea.output)
  #   dea.plot(dea.in(), dea.out(), txt = TRUE, main = ('Simple plot of pioneeR'),
  #            RTS = input$plot.rts, ORIENTATION = input$plot.orientation)
  # })
  ...
  observeEvent(input$exportplot, {
    ...
    # orca(p, file = tmpFile)
    browseURL(tmpFile)
    # webshot::webshot(p, file = tmpFile)
  })
  ...
  output$malm.dt <- renderUI({
    ...
    if (df$listwise)
      out <- div(class = 'alert alert-info', df$message)
    else
      out <- NULL
